...
# ╔═╡ df3a2d8c-039d-11ee-2303-598a06e198a0
begin
	using GLMakie
	using TetGen
	using StructArrays
...
# ╔═╡ 1b9d1931-40a4-4cfe-8af1-f97d611459b2
md"""
...
# ╔═╡ 7ef9fe28-03ec-472f-a677-8e53a310faf2
...
# ╔═╡ 6998cc7d-90d4-4891-b163-5ea3b616a08f
...
# ╔═╡ b863d820-afee-4e9a-8b9d-f60aeb5bdbf8
...
# ╔═╡ abed3930-b10c-482a-b632-45a6fd3fcebb
...
# ╔═╡ 5b2d2ca4-53db-481d-8b33-9a653e2ae169
function makeaxis3d(ctx, ele, azi)
	# the usual makeaxis3d function, extended by variable elevation and azimuth angles, only necessary in Pluto
    XMakie = ctx[:Plotter]
	ctx[:elev] = ele
	ctx[:azim] = azi
    if ctx[:scene3d] == :LScene
...
# ╔═╡ 0a38eb22-a178-43ce-93fd-5f8c15ac208a
...
# ╔═╡ 9104d65a-cb3d-42f6-9d9a-de5e47c12ba8
md"""
#### unimport support functions
#### e.g. the builder is created
"""

# ╔═╡ cd27242d-085b-460b-8db7-c1495a8e3b39
...
# ╔═╡ c93db826-40ac-4818-ba06-2ffad6943765


# ╔═╡ a87cdca9-5ae3-428f-b373-780469c730d1
md"""
### (important support) functions I wrote
#### culminating in the boundaryplot3d function
"""

# ╔═╡ 075f64c3-29a4-4c01-a10e-614b835fd3c3
...
# ╔═╡ e41bad64-6c9a-420f-8094-7531c74ab99c
function gridplot33d(ctx, TP::Type{MakieType}, ::Type{Val{3}}, grid, elev, azim)
    ...
    if !haskey(ctx, :scene)
        ...
        ctx[:scene] = makeaxis3d(ctx, elev, azim)
...
# ╔═╡ ef60f22b-b2f2-4ccb-b5a5-9fa60b5fdc88
...
# ╔═╡ 5740d84e-ca37-4bd7-898a-1bdabd3478dc
function boundaryplot3d(builder, elev, azim)
	make_mesh(pts, fcs) = 
	GeometryBasics.Mesh(
...
# ╔═╡ f82bb57c-bcb0-4ab2-813a-e6e80c340139
md"""
### playground
"""

# ╔═╡ 9f5b0d63-c0b5-4ee9-81c8-9edd02c0067c
begin
...
# ╔═╡ fcc2307f-8d5c-4c6e-8819-448b1d0a8e44
function execute_gridplot(grid, elev, azim)
	visualizer = GridVisualizer(; Plotter=GLMakie, show=true)
	ctx = visualizer[1,1]
	gridplot33d(ctx, plottertype(ctx[:Plotter]), Val{dim_space(grid)}, grid, elev, azim)
end

# ╔═╡ aed2d564-99f5-49b5-bac9-301518545381
function in_out(builder, elev, azim)
	grid = simplexgrid(builder)
	fig = Figure()
	fig[1,1]
	wireframe_boundary_clean(builder, elev, azim)
...
# ╔═╡ a8aa616f-0b17-4393-8af2-9cfb3373f51f
#boundaryplot3d(builder_SR, elev, azim)

# ╔═╡ cba7f021-62d1-420a-add6-0b269f030c5f
@bind elev PlutoUI.Slider(-90.0:90.0, 45.0, true)

# ╔═╡ 41d9ec31-d490-4503-bfc6-9a257c8b36d0
@bind azim PlutoUI.Slider(-180.0:180.0, 135.0, true)

# ╔═╡ d495505b-a870-4020-b65a-bcc4a55e54d9
boundaryplot3d(builder_IB, elev, azim)

# ╔═╡ b5873c09-5b04-4262-b8f9-b515f838f5e0
execute_gridplot(grid_SR, elev, azim)

# ╔═╡ e7de9800-c02c-4c87-a492-dc6efbca393d
...
julia_version = "1.9.0"
manifest_format = "2.0"
project_hash = "e7ca9278786048d4d662388e158b174ccef995ec"
